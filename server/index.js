...
app.post('/api/s-inscrire', function (req, res) {
	...
	db.exists('utilisateurs:' + identifiant, async function (err, reponse) {
		...
		if (reponse === 0) {
			...
			multi.hmset('utilisateurs:' + identifiant, 'id', identifiant, 'email', email, 'motdepasse', hash, 'date', date, 'nom', '', 'langue', 'en')
			multi.exec(function () {
				req.session.identifiant = identifiant
				req.session.nom = ''
				...
				req.session.cookie.expires = new Date(Date.now() + dureeSession)
				req.session.filtre = 'date-desc'
				res.json({ identifiant: identifiant, nom: '', email: email, langue: 'en', statut: 'utilisateur' })
			})
		} else {
		...
		if (reponse === 1) {
			db.hgetall('interactions:' + code, function (err, donnees) {
				...
				if (donnees.hasOwnProperty('motdepasse') && motdepasse !== '' && motdepasse === donnees.motdepasse) {
					...
					req.session.filtre = 'date-desc'
					res.json({ message: 'interaction_debloquee', code: code, identifiant: identifiant, nom: '', langue: 'en', statut: 'auteur', interactions: req.session.interactions, digidrive: req.session.digidrive })
