	...
	methods: {
		...
		gererAccordeon (indexQuestion) {
			...
			if (this.questions[indexQuestion].question === '') {
				this.$nextTick(function () {
					if (document.querySelector('#question' + indexQuestion + ' .question textarea')) {
		...
		ajouterQuestion (interaction) {
			if (interaction === 'Sondage') {
				...
				this.questions.push({ question: '', support: {}, option: 'choix-unique', items: [{ texte: '', image: '', alt: '', reponse: false }, { texte: '', image: '', alt: '', reponse: false }], reponses: '', retroaction: { correcte: '', incorrecte: '' }, points: 1000 })
			}
