...
export default {
	...
	watch: {
		validation: function (valeur) {
			if (this.options.progression === 'animateur' && valeur === 'validation') {
				this.definirScore(this.indexQuestion)
			}
		},
	...
	methods: {
		definirScore (indexQuestion) {
			...
			if (reponseCorrecte.every(i => this.reponse[indexQuestion].includes(i)) === true) {
				if (this.options.points === 'classique') {
					score = 1000
				} else {
					const temps = Math.abs((new Date().getTime() - this.date) / 1000)
					score = Math.round(1000 - (temps * 10))
				}
			} else {
				...
				if ((bonnesReponses.length - mauvaisesReponses.length) > 0) {
					if (this.options.points === 'classique') {
						score = ((1000 / reponseCorrecte.length) * (bonnesReponses.length - mauvaisesReponses.length))
					} else {
						const temps = Math.abs((new Date().getTime() - this.date) / 1000)
						score = ((Math.round(1000 - (temps * 10)) / reponseCorrecte.length) * (bonnesReponses.length - mauvaisesReponses.length))
					}
				}
			}
			this.score[indexQuestion] = score
			this.$emit('validation', { reponse: this.reponse, score: this.score, identifiant: this.identifiant, nom: this.nom })
		},
		envoyerReponse (indexQuestion) {
			if (this.reponse[indexQuestion].length > 0 && this.reponseEnvoyee[indexQuestion] !== 'reponse-envoyee') {
				this.definirScore(indexQuestion)
			}
		}
	}
}
