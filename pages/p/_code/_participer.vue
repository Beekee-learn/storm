				...
				<div class="conteneur">
					<div class="contenu">
						<div class="etoiles" v-if="etoilesScore">
							<span v-for="etoile in (parseInt(10 * (scoreTotal / (donnees.questions.length * 1000))))" :key="etoile"><i class="material-icons">star_rate</i></span>
						</div>
						<span class="score">{{ scoreTotal }}</span>
						<span class="sur">{{ $t('sur') }}</span>
						<span class="total">{{ donnees.questions.length * 1000 }}</span>
					</div>
				</div>
			</div>
		</div>
...
export default {
	...
	sockets: {
		...
		interactionfermee: function () {
			if (this.modale === 'nom' || this.modale === 'parametres') {
				this.modale = ''
			}
			this.statut = 'termine'
		},
		...
		scoreTotal () {
			...
			this.reponsesSession.forEach(function (item) {
				if (item.identifiant === this.identifiant) {
					tableauScore = item.score
				}
			}.bind(this))
			let score = 0
			tableauScore.forEach(function (points) {
				score = score + points
			})
			return score
		}
